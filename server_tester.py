...
# 生成基于时间的文件名
# 接受 文件扩展名 文件名前缀(默认为空) 制定的时间(默认当前时间)
...
def file_process(username, request_operation, json_data, bin_data, connection_socket):
    ...
    if request_operation == OP_GET:
        ...
        logger.info(f'<-- Plan: file size {file_size}, total block_index number {FIELD_TOTAL_BLOCK}.')
        connection_socket.send(
    ...
    if request_operation == OP_UPLOAD:
        if FIELD_KEY not in json_data.keys():
            # todo: 是否为 error
            logger.info(f'--> Upload file/block_index without any key.')
            logger.error(f'<-- Field "key" is missing for FILE block_index uploading.')
            connection_socket.send(
                make_response_packet(OP_UPLOAD, 410, TYPE_FILE, f'Field "key" is missing for FILE uploading.', {}))
            return
        logger.info(f'--> Upload file/block_index of "key" {json_data[FIELD_KEY]}.')
        ...
        connection_socket.send(
            make_response_packet(OP_UPLOAD, 200, TYPE_FILE, f'The block_index {block_index} is uploaded.', rval))
        return

    if request_operation == OP_DOWNLOAD:
        if FIELD_KEY not in json_data.keys():
            logger.info(f'--> Download file/block_index without any key.')
            logger.error(f'<-- Field "key" is missing for FILE block_index downloading.')
            connection_socket.send(
                make_response_packet(OP_GET, 410, TYPE_FILE, f'Field "key" is missing for FILE downloading.', {}))
            return
        logger.info(f'--> Download file/block_index of "key" {json_data[FIELD_KEY]}.')
...
def tcp_listener(server_ip, server_port):
    ...
    global logger
    '''
    这个错误是由于尝试在一个SOCK_DGRAM类型的套接字上调用listen方法而引起的。
    在你的tcp_listener函数中，你创建了一个SOCK_DGRAM类型的套接字，
    但在之后的代码中，你尝试调用listen方法，这是不允许的。
    listen方法仅适用于SOCK_STREAM类型的套接字，用于TCP连接。

    要解决这个问题，你需要将服务器套接字的类型更改为SOCK_STREAM，因为TCP连接是基于流的，而SOCK_DGRAM是基于数据报的。
